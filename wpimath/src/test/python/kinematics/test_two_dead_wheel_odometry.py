...
@pytest.fixture
def odometry():
    return TwoDeadWheelOdometry(1, 1, 0, 0, Rotation2d(), Pose2d())


def test_multiple_consecutive_updates(odometry):
    odometry.reset_position(1, 1, Rotation2d(), Pose2d())

    odometry.update(1, 1, Rotation2d())
    pose = odometry.update(1, 1, Rotation2d())

    assert pose.x == pytest.approx(0, abs=0.01)
    assert pose.y == pytest.approx(0, abs=0.01)
    assert pose.rotation().radians() == pytest.approx(0, abs=0.01)


def test_two_iterations(odometry):
    odometry.reset_position(0, 0, Rotation2d(), Pose2d())

    odometry.update(0, 0, Rotation2d())
    pose = odometry.update(0.1, 0, Rotation2d())
...
def test_gyro_angle_reset(odometry):
    gyro_angle = Rotation2d.from_degrees(90)
    odometry.reset_position(0, 0, gyro_angle, Pose2d())

    odometry.update(1, 0, gyro_angle)
    pose = odometry.update(1, 0, gyro_angle)

    assert pose.x == pytest.approx(1, abs=0.01)
    assert pose.y == pytest.approx(0, abs=0.01)
    assert pose.rotation().radians() == pytest.approx(0, abs=0.01)
...
def _test_accuracy(odometry, face_trajectory):
    ...
    if face_trajectory:
        initial_pose = trajectory.initial_pose()
        gyro_angle = initial_pose.rotation()
    else:
        initial_pose = Pose2d()
    ...
    while time < trajectory.duration():
        ...
        if face_trajectory:
            ...
            gyro_angle = state.pose.rotation() + Rotation2d(
                generator.gauss(0, 0.001)
            )
        else:
            vx = velocity * state.pose.rotation().cos()
            vy = velocity * state.pose.rotation().sin()
            omega = 0
            gyro_angle = Rotation2d(generator.gauss(0, 0.001))
